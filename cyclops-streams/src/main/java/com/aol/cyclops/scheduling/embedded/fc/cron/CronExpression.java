...
public class CronExpression {
    ...
    public ZonedDateTime nextTimeAfter(ZonedDateTime afterTime, long durationInMillis) {
        ...
        return nextTimeAfter(afterTime, afterTime.plus(durationInMillis,ChronoUnit.MILLIS));
    }

    public ZonedDateTime nextTimeAfter(ZonedDateTime afterTime, ZonedDateTime dateTimeBarrier) {
    	ZonedDateTime nextTime = afterTime;
                ...
                while (true) { // day of month
                    while (true) { // hour
                        while (true) { // minute
                            ...
                            nextTime = nextTime.plus(1, ChronoUnit.MINUTES);
        ...
        boolean matches(LocalDate dato) {
            for (FieldPart part : parts) {
                if ("L".equals(part.modifier)) {
                    ...
                    return dato.getDayOfWeek().ordinal() == part.from && dato.getDayOfMonth() > (dato.lengthOfMonth() - 7);
                } else if ("#".equals(part.incrementModifier)) {
                    if (dato.getDayOfWeek().ordinal() == part.from) {
                        int num = dato.getDayOfMonth() / 7;
                        return part.increment == (dato.getDayOfMonth() % 7 == 0 ? num : num + 1);
                    }
                    return false;
                } else if (matches(dato.getDayOfWeek().ordinal(), part)) {
                    return true;
                }
            }
            return false;
        ...
        boolean matches(LocalDate dato) {
            for (FieldPart part : parts) {
                if ("L".equals(part.modifier)) {
                    ...
                    return dato.getDayOfMonth() == (dato.lengthOfMonth() - (part.from == null ? 0 : part.from));
                } else if ("W".equals(part.modifier)) {
                    if (dato.getDayOfWeek().ordinal() <= 5) {
                        if (dato.getDayOfMonth() == part.from) {
                            return true;
                        } else if (dato.getDayOfWeek().ordinal() == 5) {
                            return dato.plusDays(1).getDayOfMonth() == part.from;
                        } else if (dato.getDayOfWeek().ordinal() == 1) {
