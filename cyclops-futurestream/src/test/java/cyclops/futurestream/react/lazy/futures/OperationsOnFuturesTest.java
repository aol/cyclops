...
public class OperationsOnFuturesTest {
	...
	protected <U> FutureStream<U> react(Supplier<U>... array) {
	    ...
	    public void testCycle() {
			for(int i=0;i<1000;i++)
	    	   Assert.assertEquals(asList(1, 1, 1, 1, 1,1),of(1).actOnFutures().cycle().limit(6).toList());

	    }
		@Test
	    public void testCycleTimes() {
	        Assert.assertEquals(asList(1, 2, 1, 2, 1, 2),of(1, 2).actOnFutures().cycle(3).toList());
	    ...
	    public void testShuffle() {
	        ...
	        Assert.assertEquals(3, s.get().actOnFutures().shuffle().toList().size());
	...
	public void testZipWithIndex() {
		...
		assertEquals(asList(tuple("a", 0L), tuple("b", 1L)), of("a", "b")
				.zipWithIndex().toList());
		Assert.assertEquals(asList(tuple("a", 0L), tuple("b", 1L), tuple("c", 2L)),
