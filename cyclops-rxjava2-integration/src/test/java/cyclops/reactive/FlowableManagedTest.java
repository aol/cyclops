...
public class FlowableManagedTest {
    ...
    @Test
    public void release(){
        FlowableManaged.managed("hello", t->close())
            .map(s->s.length())
    ...
    public void test() throws InterruptedException {
        Managed<Future<Integer>> squashed = Range.range(1, 5).lazySeq().map(this::resource).foldLeft(FlowableManaged.monoid(zipFutures(intSum)));
    ...
    public void checkOpenFlatMap(){
        Try<ManagedTest.Resource, Throwable> t = FlowableManaged.of(() -> new ManagedTest.Resource()).flatMap(r -> {
            if (r.open)
                return Managed.managed(new ManagedTest.Resource());
            throw new RuntimeException("boo!");
        }).run();
