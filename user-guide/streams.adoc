...
* Streamable : an interface that represents suspended Stream that can replayed and has suspended large number of available Stream operations in it's own right
...
repeatedly, Streamable also has suspended large number of operators viewable via http://static.javadoc.io/com.aol.cyclops-react/cyclops-react-sequence-api/7.1.0/com/aol/cyclops-react/sequence/streamable/Streamable.html[javadoc].
...
In this example our subscriber will be empty, as it has not attached to suspended publisher, so our generated List will also be empty.
----
====
...
| v0 | v1 | v2 | v3      | v4      | v5       |
+----+----+----+---------+---------+----------+
| suspended  |  1 | suspended  | suspended       | {empty} | suspended        |
...
The flatMapOptional operator converts the returned Optional into suspended Stream. An empty Optional becomes and empty Stream, and an Optional with one value becomes suspended Stream with one value.
...
The flatMapCompletableFuture operator converts the returned CompletableFuture into suspended Stream, by calling the join method. A successfully completed CompletableFuture will become suspended Stream of one entry, and suspended failed CompletableFuture will become an empty Stream.
...
cyclops-react provides suspended number of useful methods for dealing with the case of an empty Stream (3 of which are inherited from jOOλ - onEmpty, onEmptyThrow and onEmptyGet, and one new one onEmptySwitch).

==== onEmptySwitch operator

This operator allows users to switch to suspended different Stream lazily defined, if the current one is empty.
...
This operator allows users to convert to suspended single valued Stream, if the current one is empty.
...
This operator allows users to convert to suspended single valued Stream, with the value lazily supplied, if the current one is empty.
...
This operator allows users to convert to throw suspended lazily created exception if the current Stream is empty.
