...
.forEachWithError with a SequenceM
====
[source,java]
----
...
Subscription s = SequenceM.of(1,2,3,4)
                          .map(this::load)
...
Integrated as of cyclops 7.2.0
...
[TIP]
====
The jOOλ functions are exceptionally powerful and flexible, but also consume the Stream. This means they will not perform as well as the simpler (but less powerful) batchBy, windowBy and sliding functions in cyclops. They are also not suitable for use in infinitely large Streams.
====
...
The removeAll operator removes all supplied values from the Stream.

.removeAll
====
[source,java]
----
SequenceM.of(1, 2,3).removeAll(2,3);
//(1)
====
...
The retainAll operator retains only the supplied values in the Stream.

.retainAll
====
[source,java]
----
SequenceM.of(1, 2,3).retainAll(2,3);
//(2,3)
====
...
SequenceM.of(1,2,3).foldLeft(0,(a,b)->a+b);
//6
...
The Reducers class contains a number of cyclops Monoid instances, Monoid is a class that maps to the method signature of reduce / foldLeft / foldRight / scanLeft / scanRight. The Reducers class contains canned reduction operations for String concatonation, Immutable List concatonation, Numeric reduction etc.
...
----
The map to type operator accepts a cyclops Monoid instance, and uses the mapToType function on that interface to enforce the type accepted by the Monoid.
[source,java]
----
...
SequenceM.of(1,2,3).foldRight(0,(a,b)->a+b);
//6
...
List<Integer> result = SequenceM.of(1,2,3,4)).reduce(Arrays.asList(sum,mult) );
//List[10,24]
----
====
...
SequenceM extends the jOOλ Collectable interface, this provides a range of useful additional terminal operations such as mean, mode, median & percentile and advanced Collection operators that enable you to in a type safe manner perform mutliple different types of mutable reductions simulatanouesly on a Stream.

.mulitple mutable reduction (Collection)
====

[source,java]
----
 Tuple3<List<Integer>,Integer, Integer> aggregations = SequenceM.of(1,2,3,4)
          .collect(Collectors.toList(),
                 Collectors.averagingInt(i->i), Collectors.counting());

//aggregations = Tuple3[List[1,2,3,4],2,4]
----
====

==== FutureCollectable interface 

cyclops additionally provides a FutureCollectable interface that it avaiable to SequenceM and StreamUtils via the futureOperations operator. The FutureCollectable interface provides asynchronous versions of the jOOλ Collectable termina operations.


.async mulitple mutable reduction (Collection)
====

[source,java]
----
Executor exec = Executors.newFixedThreadPool(1);
 CompletableFuture<Tuple3<List<Integer>,Integer, Integer>> aggregations = SequenceM.of(1,2,3,4)
                         .futureOperations(exec)
                         .collect(Collectors.toList(),
                 Collectors.averagingInt(i->i), Collectors.counting());
...
SequenceM.of(1,2,3).toList(()-> new LinkedList())
//LinkedList[1,2,3]
...
Cyclops provides operators to lazily create a collection from the Stream. The values are only pulled through the Stream as the lazy collection is used. toConcurrentLazyCollection synchronizes iteration through the underlying Stream.
...
The anyM operator wraps the SequenceM in the cyclops anyM wrapper class that can provide a common api over any monad type.

[NOTE]
====
A monad is a fluent, generic wrapper type (that obeys the monad laws) - like Stream, Optional, CompleteableFuture in Java as well as Try & FeatureToggle in cyclops.
====

AnyM keeps type information for the underlying data - so if we call anyM() on a SequenceM of integers the result is an AnyM<Integer>. AnyM facilates writing common code that can accept any monad type.

The toStreamable operator converts the SequenceM to a caching Streamable.

.toStreamable / anyM
====
[source,java]
----
Streamable<String> caching = SequenceM.of("hello","world")
												.toStreamable();
...
AnyM<String> anyM = SequenceM.of("hello","world")
											.anyM();


anyM.map(v->v+"!");
//AnyM[SequenceM["hello!","world!"]]
----
====
