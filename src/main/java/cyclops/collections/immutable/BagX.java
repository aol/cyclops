...
public interface BagX<T> extends To<BagX<T>>,PBag<T>, PersistentCollectionX<T>, OnEmptySwitch<T, PBag<T>> {
     ...
     * </pre>
     * 
     * @param bagX toNested narrowK generic type
    ...
    public static BagX<Integer> range(final int start, final int end) {
        return ReactiveSeq.range(start, end).to().bagX(Conversion.LAZY);
    }
    ...
    public static BagX<Long> rangeLong(final long start, final long end) {
        return ReactiveSeq.rangeLong(start, end).to().bagX(Conversion.LAZY);
    ...
    static <U, T> BagX<T> unfold(final U seed, final Function<? super U, Optional<Tuple2<T, U>>> unfolder) {
        return ReactiveSeq.unfold(seed, unfolder).to().bagX(Conversion.LAZY);
    }

    /**
     * Generate a BagX from the provided Supplier up toNested the provided limit number of times
     * 
     * @param limit Max number of elements toNested generate
     * @param s Supplier toNested generate BagX elements
     * @return BagX generated from the provided Supplier
     */
    public static <T> BagX<T> generate(final long limit, final Supplier<T> s) {

        return ReactiveSeq.generate(s)
                          .limit(limit).to().bagX(Conversion.LAZY);
    }
    /**
     * Generate a BagX from the provided value up toNested the provided limit number of times
     * 
     * @param limit Max number of elements toNested generate
    ...
    /**
     * Create a BagX by iterative application of a function toNested an initial element up toNested the supplied limit number of times
     * 
     * @param limit Max number of elements toNested generate
     ...
     * 
     * @param publisher
     *            toNested construct BagX from
     * @return BagX
     */
    public static <T> BagX<T> fromPublisher(final Publisher<? extends T> publisher) {
        return Spouts.from((Publisher<T>) publisher).to().bagX(Conversion.LAZY);
    }
