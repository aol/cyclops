...
public class Streams {
    ...
    /**
     * Perform a For Comprehension over a LazyList, accepting 3 generating functions.
     ...
     *
          forEach4(IntStream.range(1,10).boxed(),
                   a-> LazyList.iterate(a,i->i+1).limit(10),
                  (a,b) -> LazyList.<Integer>of(a+b),
                  (a,b,c) -> LazyList.<Integer>just(a+b+c),
                  Tuple::tuple)
     *
     * }
     * </pre>
     *
     * @param value1 top level LazyList
     * @param value2 Nested LazyList
     * @param value3 Nested LazyList
     * @param value4 Nested LazyList
     * @param yieldingFunction  Generates a result per combination
     * @return LazyList with an element per combination of nested publishers generated by the yielding function
     */
    ...
    /**
     * Perform a For Comprehension over a LazyList, accepting 3 generating function.
     * This results in a four level nested internal iteration over the provided Publishers.
     * <pre>
     * {@code
     *
    ...
    /**
     * Perform a For Comprehension over a LazyList, accepting 2 generating function.
    ...
    /**
     * Perform a For Comprehension over a LazyList, accepting an additonal generating function.
     ...
     *  forEach(IntStream.range(1, 10).boxed(),
     *          i -> LazyList.range(i, 10), Tuple::tuple)
    ...
    /**
     * Create an Optional containing a List materialized from a LazyList
     * 
     * <pre>
     * {@code 
     *   Optional<ListX<Integer>> opt = Streams.streamToOptional(LazyList.of(1,2,3));
     *   
     *   //Optional[[1,2,3]]
     * 
     * }
    ...
    /**
     * Convert an Optional to a LazyList
     * 
     * <pre>
     * {@code 
     *     LazyList<Integer> reactiveStream = Streams.optionalToStream(Optional.of(1));
     *     //LazyList[1]
     *     
     *     LazyList<Integer> zero = Streams.optionalToStream(Optional.zero());
     *     //LazyList[]
     * }
     * </pre>
     * 
     * @param optional Optional to convert to a LazyList
     * @return LazyList with a singleUnsafe value (if present) created from an Optional
     */
    ...
    /**
     * Create a CompletableFuture containing a List materialized from a LazyList
     * 
     * @param stream To convert into an Optional
    ...
    /**
     * Convert a CompletableFuture to a LazyList
     * 
     * @param future CompletableFuture to convert
     * @return  LazyList with a singleUnsafe value created from a CompletableFuture
     */
     ...
     * <pre>
     * @{code
     *     Subscription next = Streams.forEach(LazyList.of(1,2,3,4),2,System.out::println);
     *          
     ...
     * </pre>
     * 
     * @param Stream - the LazyList to consume data from
     ...
     * <pre>
     * @{code
     *     Subscription next = Streams.forEach(LazyList.of(()->1,()->2,()->{throw new RuntimeException()},()->4)
     ...
     * Streams.schedule(LazyList.generate(()->"next job:"+formatDate(new Date()))
     *            .map(this::processJob)
     ...
     * <pre>
     * {@code 
     * HotStream<Data> dataStream = Streams.schedule(LazyList.generate(()->"next job:"+formatDate(new Date()))
     *            							  .map(this::processJob)
     *            							  ,"0 20 * * *",Executors.newScheduledThreadPool(1)));
     * 
     * 
     ...
     *  Streams.scheduleFixedDelay(LazyList.generate(()->"next job:"+formatDate(new Date()))
     *            .map(this::processJob)
     ...
     * <pre>
     * {@code 
     * HotStream<Data> dataStream = Streams.scheduleFixedDelay(LazyList.generate(()->"next job:"+formatDate(new Date()))
     *            							  .map(this::processJob)
     *            							  ,60_000,Executors.newScheduledThreadPool(1)));
     * 
     * 
     ...
     * 
     * @param stream the reactiveStream to schedule element processing on
     * @param delay Between last element completes passing through the LazyList until the next one starts
     * @param ex ScheduledExecutorService
     * @return Connectable HotStream of emitted from scheduled LazyList
     */
     ...
     *  Streams.scheduleFixedRate(LazyList.generate(()->"next job:"+formatDate(new Date()))
     *            .map(this::processJob),
     *            60_000,Executors.newScheduledThreadPool(1)));
     * }
     * </pre>
     ...
     * <pre>
     * {@code 
     * HotStream<Data> dataStream = Streams.scheduleFixedRate(LazyList.generate(()->"next job:"+formatDate(new Date()))
     *            							  .map(this::processJob)
     *            							  ,60_000,Executors.newScheduledThreadPool(1)));
     * 
     * 
    ...
    /**
     * Makes four copies of a LazyList
     * Buffers intermediate values, leaders may change positions so a limit
     * can be safely applied to the leading reactiveStream. Not thread-safe.
     * 
     * <pre>
     ...
     * @return LazyList with LazyList appended
     */
    ...
    /**
     * Prepend LazyList to this LazyList
     * 
     * <pre>
     * {@code 
     ...
     * @return LazyList with LazyList prepended
     */
    ...
    /**
     * Append values to the take of this LazyList
     * <pre>
     * {@code 
     ...
     * @return LazyList with appended values
     */
     ...
     * @return LazyList with values prepended
     */
     ...
     * <pre>
     * {@code 
     * List<String> result = 	Streams.deleteBetween(LazyList.of(1,2,3,4,5,6),2,4)
    										.map(it ->it+"!!")
    										.collect(CyclopsCollectors.toList());
     ...
     * <pre>
     * {@code 
     * List<String> result = 	Streams.insertAtS(LazyList.of(1,2,3),1,of(100,200,300))
    										.map(it ->it+"!!")
    										.collect(CyclopsCollectors.toList());
     ...
     * @return newly conjoined LazyList
     */
     ...
     * <pre>
     * {@code 
     *   		List<Integer> list = Streams.cycle(LazyList.of(1,2,2),Reducers.toCountInt(),3)
     * 										.
     * 										.collect(CyclopsCollectors.toList());
     * 	//is asList(3,3,3);
     *   }
     ...
     * <pre>
     * {@code 
     *  LazyList<String> helloWorld = LazyList.of("hello","world","last");
    ...
    /**
     * skip elements in LazyList until Predicate holds true
     * 	<pre>
     * {@code  Streams.skipUntil(LazyList.of(4,3,6,7),i->i==6).collect(CyclopsCollectors.toList())
     *  // [6,7]
     *  }</pre>
    
     * @param stream LazyList to skip elements from
     * @param predicate to applyHKT
     * @return LazyList with elements skipped
     */
    ...
    /**
     * skip elements in a LazyList while Predicate holds true
     * 
     * <pre>
     * 
     * {@code  Streams.skipWhile(LazyList.of(4,3,6,7).sorted(),i->i<6).collect(CyclopsCollectors.toList())
     *  // [6,7]
     *  }</pre>
     * @param stream
     * @param predicate
     ...
     * <pre>
     * {@code Streams.limitWhile(LazyList.of(4,3,6,7).sorted(),i->i<6).collect(CyclopsCollectors.toList());
     * //[4,3]
     * }
     * </pre>
     * @param stream
    ...
    /**
     * Take elements from a LazyList until the predicate holds
     *  <pre>
     * {@code Streams.limitUntil(LazyList.of(4,3,6,7),i->i==6).collect(CyclopsCollectors.toList());
     * //[4,3]
     * }
     * </pre>
     * @param stream
     ...
     * <pre>
     * {@code 
     * assertThat(Streams.reverse(LazyList.of(1,2,3)).collect(CyclopsCollectors.toList())
    			,equalTo(Arrays.asList(3,2,1)));
     * }
     * </pre>
     * 
     * @param stream LazyList to reverse
    ...
    /**
     * Create a reversed LazyList from a List
     * <pre>
     * {@code 
     ...
     * </pre>
     * 
     * @param list List to create a reversed LazyList from
     * @return Reversed LazyList
     */
    ...
    /**
     * Create a new LazyList that infiniteable cycles the provided LazyList
     * 
     * <pre>
     * {@code 		
     * assertThat(Streams.cycle(LazyList.of(1,2,3))
     * 						.limit(6)
     * 						.collect(CyclopsCollectors.toList()),
     * 								equalTo(Arrays.asList(1,2,3,1,2,3)));
    	}
     * </pre>
     * @param s LazyList to cycle
    ...
    /**
     * Create a LazyList that infiniteable cycles the provided Streamable
    ...
    /**
     * Create a LazyList that finitely cycles the provided Streamable, provided number of times
     * 
     * <pre>
     * {@code 
     ...
     * }
     * </pre>
     * @param it Iterable to convert to a LazyList
     * @return LazyList from iterable
     */
     ...
     * }
     * </pre>
     * @param it Iterator to convert to a LazyList
     * @return LazyList from iterator
     */
    ...
    /**
     * Concat an Object and a LazyList
     * If the Object is a LazyList, Streamable or Iterable will be converted (or left) in LazyList form and concatonated
     * Otherwise a new LazyList.of(o) is created
     * 
     * @param o Object to concat
     * @param stream  LazyList to concat
     * @return Concatonated LazyList
     */
     ...
     *  Monoid<Integer> sum = Monoid.of(0,(a,b)->a+b);
    	...
    	val result = Streams.reduce(LazyList.of(1,2,3,4),Arrays.asList(sum,mult));
    			
    	 
    	assertThat(result,equalTo(Arrays.asList(10,24)));
    	}</pre>
     * 
     * @param stream LazyList to reduce
     * @param reducers Reducers to reduce LazyList
     * @return Reduced LazyList values as List entries
     */
     ...
     *  Monoid<String> concat = Monoid.of("",(a,b)->a+b);
    	...
    	assertThat(Streams.reduce(LazyList.of("hello", "world", "woo!"),LazyList.of(concat,join))
     ...
     *  int count =0;
     *  
    	assertThat(Streams.cycleWhile(LazyList.of(1,2,2)
    										,next -> count++<6 )
     ...
     * {@code 
     * 	count =0;
    	assertThat(Streams.cycleUntil(LazyList.of(1,2,2,3)
    										,next -> count++>10 )
    ...
    /**
     * Generic zip function. E.g. Zipping a LazyList and a Sequence
     * 
     * <pre>
     * {@code 
     * LazyList<List<Integer>> zipped = Streams.zip(LazyList.of(1,2,3)
     ...
     * @param zipper
     *            Zipping function
     * @return LazyList zipping two Monads
    */
    ...
    /**
     *  Generic zip function. E.g. Zipping a LazyList and an Optional
     * 
     * <pre>
     * {@code
     * LazyList<List<Integer>> zipped = Streams.zip(LazyList.of(1,2,3)
    ...
    /**
     * Zip this Monad with a LazyList
     * 
       <pre>
       {@code 
       LazyList<List<Integer>> zipped = Streams.zipStream(LazyList.of(1,2,3)
    											,LazyList.of(2,3,4),
    												(a,b) -> Arrays.asList(a,b));
     ...
     * 
     * @param second
     *            LazyList to zip with
     * @param zipper
     *            Zip funciton
     * @return This monad zipped with a LazyList
     */
    ...
    /**
     * Create a sliding view over this LazyList
     * <pre>
     * {@code 
     * List<List<Integer>> list = Streams.sliding(LazyList.of(1,2,3,4,5,6)
    											,2,1)
    								.collect(CyclopsCollectors.toList());
     ...
     * @return LazyList with sliding view
     */
     ...
     * @param windowSize
     *            Size of sliding window
     * @return LazyList with sliding view over monad
     */
     ...
     * </pre> 
     * 
     * @param stream LazyList to create sliding view on
    ...
    /**
     * Group elements in a LazyList by size
     * 
       <pre>
       {@code 
     *  List<List<Integer>> list = Streams.grouped(LazyList.of(1,2,3,4,5,6)
                                                        ,3)
                                                    .collect(CyclopsCollectors.toList());
     ...
     * </pre>
     * @param stream LazyList to group
     * @param groupSize
     *            Size of each Group
     ...
     * 
       <pre>
       {@code 
     *  List<SetX<Integer>> list = Streams.grouped(LazyList.of(1,2,3,4,5,6)
                                                        ,3,()->SetX.zero())
                                                    .collect(CyclopsCollectors.toList());
     ...
     * 					Streams.scanLeft(LazyList.of("a", "b", "c"),Reducers.toString(""))
     * 			.collect(CyclopsCollectors.toList());
     *         
     *         }
     * </pre>
     ...
     * <pre>
     * {@code 
     *  assertTrue(Streams.xMatch(LazyList.of(1,2,3,5,6,7),3, i->i>4));
     * }
     * </pre>
     * 
     */
     ...
     * 
     * <pre>{@code 
     * assertTrue(Streams.startsWith(LazyList.of(1,2,3,4),Arrays.asList(1,2,3)));
     * }</pre>
     * 
     * @param iterable
     ...
     * 	<pre>
     * {@code
     * 		 assertTrue(Streams.startsWith(LazyList.of(1,2,3,4),Arrays.asList(1,2,3).iterator()))
     * }</pre>
    ...
    public final static <T> ReactiveSeq<T> reactiveSeq(final Stream<? super T> stream, final Optional<ReversableSpliterator> rev) {
        if (stream instanceof ReactiveSeq)
          ...
          //  return new StreamX<T>((LazyList<T>)
            //                              stream, rev);
     ...
     * 			equalTo( Streams.intersperse(LazyList.of(1, 2, 3, 4),0));
     * }
     * </pre>
     */
     ...
     *      equalTo( Streams.ofType(LazyList.of(1, "a", 2, "b", 3,Integer.class));
     * 
     */
    @SuppressWarnings("unchecked")
     ...
     * <pre>
     * {@code
     * Streams.cast(LazyList.of(1, "a", 2, "b", 3),Integer.class)
     ...
     * <pre>
     * {@code 
     * 	assertThat(Streams.flatMapCollection(LazyList.of(20),i->Arrays.asList(1,2,i))
     * 								.collect(CyclopsCollectors.toList()),
     ...
     * <pre>
     * {@code 
     * 	assertThat(Streams.flatMapStream(LazyList.of(1,2,3),
     * 							i->LazyList.of(i)).collect(CyclopsCollectors.toList()),
     * 							equalTo(Arrays.asList(1,2,3)));
     ...
     * <pre>
     * {@code 
     * 	 assertThat(Streams.flatMapOptional(LazyList.of(1,2,3,null),
     * 										Optional::ofNullable)
     * 										.collect(CyclopsCollectors.toList()),
     * 										equalTo(Arrays.asList(1,2,3)));
     ...
     *<pre>
     * {@code 
     * 	assertThat(Streams.flatMapCompletableFuture(LazyList.of(1,2,3),
     ...
     * <pre>
     * {@code 
     *   List<Character> result = Streams.liftAndBindCharSequence(LazyList.of("input.file"),
    								.i->"hello world")
    								.toList();
     ...
     * {@code
     * 
    	List<String> result = Streams.liftAndBindFile(LazyList.of("input.file")
     ...
     * <pre>
     * {@code 
     * List<String> result = Streams.liftAndBindURL(LazyList.of("input.file")
     ...
     * 
     * <pre>
     * List<String> result = Streams.liftAndBindBufferedReader(LazyList.of("input.file")
    ...
    /**
     * Lazily constructs a Collection from specified LazyList. Collections iterator may be safely used
    ...
    */
    ...
    /**
     * Group data in a LazyList using knowledge of the current batch and the next entry to determing grouping limits
     * 
     * @see ReactiveSeq#groupedStatefullyUntil(BiPredicate)
     * 
     * @param stream LazyList to group
     * @param predicate Predicate to determine grouping
     * @return LazyList grouped into Lists determined by predicate
     */
    ...
    /**
     * Group a LazyList while the supplied predicate holds
     * 
     ...
     * @return LazyList grouped into Collections determined by predicate
     */
    ...
    /**
     * Group a LazyList until the supplied predicate holds
     * 
    ...
    /**
     * Group a LazyList by size and time constraints
     * 
     ...
     * @return LazyList grouped by time and size
     */
     ...
     * 
     * @param stream LazyList to emit one element per time period from
     * @param time  Time period
     * @param t Time Pure
     * @return LazyList with slowed emission
     */
    ...
    public static class Instances {
         ...
         * <pre>
         * {@code
         *  StreamKind<Integer> list = Streams.functor().map(i->i*2, StreamKind.widen(LazyList.of(1,2,3));
         *
         *  //[2,4,6]
         *
         *
        ...
        .ap(widen(LazyList.of(l1(this::multiplyByTwo))),widen(LazyList.of(1,2,3)));
         *
         * //[2,4,6]
         * }
         * </pre>
        ...
        .flatMap(i->widen(StreamX.range(0,i)), widen(LazyList.of(1,2,3)))
        .convert(StreamKind::narrow);
         * }
         * </pre>
         *
        ...
        .plus(StreamKind.widen(LazyList.of()), StreamKind.widen(LazyList.of(10)))
        .convert(StreamKind::narrow);
        //LazyList.of(10))
         *
         * }
        ...
        .foldLeft(0, (a,b)->a+b, StreamKind.widen(LazyList.of(1,2,3,4)));
    ...
    /**
     * Simulates Higher Kinded Types for LazyList's
     *
     * StreamKind is a LazyList and a Higher Kinded Type (stream,T)
     *
     * @author johnmcclean
     *
     * @param <T> Data type stored within the LazyList
     */
    ...
    public static interface StreamKind<T> extends Higher<stream, T>, Stream<T> {
        ...
        /**
         * Convert a LazyList to a simulated HigherKindedType that captures LazyList nature
         * and LazyList element data type separately. Recover via @see StreamKind#narrow
         *
         * If the supplied LazyList implements StreamKind it is returned already, otherwise it
         * is wrapped into a LazyList implementation that does implement StreamKind
         *
         * @param Stream LazyList to widen to a StreamKind
         ...
         *
         * @param stream HTK encoded type containing  a LazyList to widen
         * @return HKT encoded type with a widened LazyList
         */
        ...
        /**
         * Convert the raw Higher Kinded Type for LazyList types into the StreamKind type definition class
         *
         * @param stream HKT encoded LazyList into a StreamKind
         * @return StreamKind
         */
        ...
        /**
         * Convert the HigherKindedType definition for a LazyList into
         *
