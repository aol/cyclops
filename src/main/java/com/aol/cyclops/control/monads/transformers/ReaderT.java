...
public interface ReaderT<T, R> extends Publisher<T> {
    ...
    default <B> ReaderT<T, B> bind(final Function<? super R, ReaderT<? extends T, B>> f) {
    ...
    public static <T, U, R> Function<ReaderT<T, U>, ReaderT<T, R>> lift(final Function<? super U, ? extends R> fn) {
        return optTu -> optTu.map(input -> fn.apply(input));
    }

    public static <T, U1, U2, R> BiFunction<ReaderT<T, U1>, ReaderT<T, U2>, ReaderT<T, R>> lift2(
            final BiFunction<? super U1, ? super U2, ? extends R> fn) {
        return (optTu1, optTu2) -> optTu1.bind(input1 -> optTu2.map(input2 -> fn.apply(input1, input2)));
    }

    public static <A, R> ReaderT<A, R> of(final AnyM<Reader<A, R>> monads) {
        return (ReaderT<A, R>) Matchables.anyM(monads)
                                         .visit(v -> ReaderTValue.<A, R> of(v), s -> ReaderTSeq.<A, R> of(s));

    }

    public static <A, R> ReaderTSeq<A, R> fromIterable(final Iterable<Reader<A, R>> iterableOfEvals) {
        return ReaderTSeq.of(AnyM.fromIterable(iterableOfEvals));
    }

    public static <A, R> ReaderTSeq<A, R> fromStream(final Stream<Reader<A, R>> streamOfEvals) {
        return ReaderTSeq.of(AnyM.fromStream(streamOfEvals));
    }

    public static <A, R> ReaderTSeq<A, R> fromPublisher(final Publisher<Reader<A, R>> publisherOfEvals) {
        return ReaderTSeq.of(AnyM.fromPublisher(publisherOfEvals));
    }

    public static <A, R, V extends MonadicValue<Reader<A, R>>> ReaderTValue<A, R> fromValue(final V monadicValue) {
        return ReaderTValue.fromValue(monadicValue);
    }

    public static <A, R> ReaderTValue<A, R> fromOptional(final Optional<Reader<A, R>> optional) {
        return ReaderTValue.of(AnyM.fromOptional(optional));
    }

    public static <A, R> ReaderTValue<A, R> fromFuture(final CompletableFuture<Reader<A, R>> future) {
        return ReaderTValue.of(AnyM.fromCompletableFuture(future));
    }

    public static <A, R> ReaderTValue<A, R> fromIterableValue(final Iterable<Reader<A, R>> iterableOfEvals) {
