...
public interface EvalT<T> extends Publisher<T>, Functor<T>, Filterable<T>, ToStream<T> {
    ...
    * }
     ...
     */
    default <B> EvalT<B> bind(final Function<? super T, EvalT<? extends B>> f) {
     ...
     */
    public static <U, R> Function<EvalT<U>, EvalT<R>> lift(final Function<? super U, ? extends R> fn) {
     ...
     * @return Function that accepts and returns an MaybeT
     */
    public static <U1, U2, R> BiFunction<EvalT<U1>, EvalT<U2>, EvalT<R>> lift2(final BiFunction<? super U1, ? super U2, ? extends R> fn) {
     ...
     * @return MaybeT
     */
    public static <A> EvalT<A> fromAnyM(final AnyM<A> anyM) {
     ...
     * @return MaybeT
     */
    public static <A> EvalT<A> of(final AnyM<Eval<A>> monads) {
        return Matchables.anyM(monads)
                         .visit(v -> EvalTValue.of(v), s -> EvalTSeq.of(s));

    }

    public static <A> EvalTValue<A> fromAnyMValue(final AnyMValue<A> anyM) {
        return EvalTValue.fromAnyM(anyM);
    }

    public static <A> EvalTSeq<A> fromAnyMSeq(final AnyMSeq<A> anyM) {
        return EvalTSeq.fromAnyM(anyM);
    }

    public static <A> EvalTSeq<A> fromIterable(final Iterable<Eval<A>> iterableOfEvals) {
        return EvalTSeq.of(AnyM.fromIterable(iterableOfEvals));
    }

    public static <A> EvalTSeq<A> fromStream(final Stream<Eval<A>> streamOfEvals) {
        return EvalTSeq.of(AnyM.fromStream(streamOfEvals));
    }

    public static <A> EvalTSeq<A> fromPublisher(final Publisher<Eval<A>> publisherOfEvals) {
    ...
    public static <A> EvalTValue<A> fromOptional(final Optional<Eval<A>> optional) {
        return EvalTValue.of(AnyM.fromOptional(optional));
    }

    public static <A> EvalTValue<A> fromFuture(final CompletableFuture<Eval<A>> future) {
        return EvalTValue.of(AnyM.fromCompletableFuture(future));
    }

    public static <A> EvalTValue<A> fromIterableValue(final Iterable<Eval<A>> iterableOfEvals) {
     ...
     */
    @Override
    default <U> EvalT<U> cast(final Class<? extends U> type) {
     ...
     */
    @Override
    default <R> EvalT<R> trampoline(final Function<? super T, ? extends Trampoline<? extends R>> mapper) {
     ...
     */
    @Override
    default <R> EvalT<R> patternMatch(final Function<CheckValue1<T, R>, CheckValue1<T, R>> case1, final Supplier<? extends R> otherwise) {
