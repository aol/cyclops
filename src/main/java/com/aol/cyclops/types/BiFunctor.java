...
public interface BiFunctor<T1, T2> {
     ...
     */
    default BiFunctor<T1, T2> bipeek(final Consumer<? super T1> c1, final Consumer<? super T2> c2) {
        return bimap(input -> {
            c1.accept(input);
            return input;
        } , input -> {
     ...
     * @return
     */
    default <U1, U2> BiFunctor<U1, U2> bicast(final Class<U1> type1, final Class<U2> type2) {
     ...
     * @return New BiFunctor containing transformed data
     */
    default <R1, R2> BiFunctor<R1, R2> bitrampoline(final Function<? super T1, ? extends Trampoline<? extends R1>> mapper1,
