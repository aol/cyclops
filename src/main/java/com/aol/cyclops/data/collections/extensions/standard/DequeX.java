...
public interface DequeX<T> extends Deque<T>, MutableCollectionX<T>, OnEmptySwitch<T, Deque<T>> {
     ...
     */
    static <U, T> DequeX<T> unfold(U seed, Function<? super U, Optional<Tuple2<T, U>>> unfolder) {
     ...
     */
    public static <T> DequeX<T> iterate(long limit, final T seed, final UnaryOperator<T> f) {
     ...
     */
    @Override
    default <R> DequeX<R> patternMatch(Function<CheckValue1<T, R>, CheckValue1<T, R>> case1, Supplier<? extends R> otherwise) {
     ...
     */
    @Override
    default <T2, T3, T4> DequeX<Tuple4<T, T2, T3, T4>> zip4(Stream<? extends T2> second, Stream<? extends T3> third, Stream<? extends T4> fourth) {
