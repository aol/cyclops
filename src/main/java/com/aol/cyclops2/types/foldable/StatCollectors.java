...
public interface StatCollectors<T> {
    ...
    default <R1, R2, A1, A2> Tuple2<R1, R2> collect(Collector<? super T, A1, R1> c1, Collector<? super T, A2, R2> c2) {
        return stream().collect(Collector.of(() -> Tuple.tuple(c1.supplier().get(),c2.supplier().get()),
                (t2, next) -> {
                    c1.accumulator().accept(t2.v1, next);
                    c2.accumulator().accept(t2.v2, next);
                },(t2, t2b) -> Tuple.tuple(c1.combiner().apply(t2.v1, t2b.v1),c2.combiner().apply(t2.v2, t2b.v2)),
                t2 -> Tuple.tuple(c1.finisher().apply(t2.v1),c2.finisher().apply(t2.v2))));
    }
    default <R1, R2, R3, A1, A2, A3> Tuple3<R1, R2, R3> collect(Collector<? super T, A1, R1> c1, Collector<? super T, A2, R2> c2, Collector<? super T, A3, R3> c3) {
        return stream().collect(Collector.of(() -> Tuple.tuple(c1.supplier().get(),c2.supplier().get(),c3.supplier().get()),
                (t3, next) -> {
                    c1.accumulator().accept(t3.v1, next);
                    c2.accumulator().accept(t3.v2, next);
                    c3.accumulator().accept(t3.v3, next);
                },(t3, t3b) -> Tuple.tuple(c1.combiner().apply(t3.v1, t3b.v1),c2.combiner().apply(t3.v2, t3b.v2),c3.combiner().apply(t3.v3, t3b.v3)),
                t3 -> Tuple.tuple(c1.finisher().apply(t3.v1),c2.finisher().apply(t3.v2),c3.finisher().apply(t3.v3))));
    }
    default long count(){
