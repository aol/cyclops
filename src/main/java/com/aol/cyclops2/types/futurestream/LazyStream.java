...
import com.aol.cyclops2.internal.react.stream.Runner;
import com.aol.cyclops2.react.collectors.lazy.EmptyCollector;
import com.aol.cyclops2.react.collectors.lazy.IncrementalReducer;
...
public interface LazyStream<U> extends BlockingStream<U> {
    ...
    default <A, R> R run(final Collector<U, A, R> collector) {
        if (getLastActive().isSequential()) {
            //if single threaded we can simply push from each Future into the collection toNested be returned
