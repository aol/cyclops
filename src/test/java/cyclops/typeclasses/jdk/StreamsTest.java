...
public class StreamsTest {
    ...
    public void functor(){
        ...
        StreamKind<Integer> list = Streams.Instances.unit()
                                     .unit("hello")
                                     .then(h->Streams.Instances.functor().map((String v) ->v.length(), h))
                                     .convert(StreamKind::narrowK);
    ...
    public void applicative(){
        ...
        StreamKind<Integer> list = Streams.Instances.unit()
                                     .unit("hello")
                                     .then(h->Streams.Instances.functor().map((String v) ->v.length(), h))
                                     .then(h->Streams.Instances.zippingApplicative().ap(listFn, h))
                                     .convert(StreamKind::narrowK);
    ...
    public void monad(){
        ...
        StreamKind<Integer> list = Streams.Instances.unit()
                                     .unit("hello")
                                     .then(h->Streams.Instances.monad().flatMap((String v) ->Streams.Instances.unit().unit(v.length()), h))
                                     .convert(StreamKind::narrowK);
    ...
    public void monadZeroFilter(){
        ...
        StreamKind<String> list = Streams.Instances.unit()
                                     .unit("hello")
                                     .then(h->Streams.Instances.monadZero().filter((String t)->t.startsWith("he"), h))
                                     .convert(StreamKind::narrowK);
