...
public class AnyMValueTest {
	...
	public void testFlatMapFirst() {
		List actualList = AnyM.fromMaybe(Maybe.just(10)).flatMapIe(i->AnyM.fromList(ListX.of(i,20,30))).stream().toList();
	...
	public void testFlatMapFirst2() {
		List actualList = AnyM.fromMaybe(Maybe.just(-100)).flatMapIe(i->AnyM.fromStream(ReactiveSeq.of(i,20,30))).stream().toList();
		assertEquals(actualList.size(), 1);
