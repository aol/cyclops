...
public class HamcrestTest {
	...
	public void hamcrest1(){
		Matching.whenValues(c->c.isMatch(hasItem("hello2")).thenConsume(System.out::println))
	...
	public void hasAnyOrderTest(){
		assertTrue(Matching.<Boolean>whenValues(c->c.isMatch(containsInAnyOrder("world","hello"))
											.thenApply(list -> true))
							.apply(Arrays.asList("hello","world")).get());
	}
	@Test
	public void hamcrestWithExtractor(){
		assertTrue(Matching.when().extract(Extractors.at(1))
										.isMatch(is("world"))
										.thenApply(list -> true)
	...
	public void postExtractorDoc(){
		Matching.when().isMatch(is(not(empty()))).thenExtract(get(1))
					.thenConsume(System.out::println)
				.match(Arrays.asList(20303,"20303 is passing",true));
	}
	@Test
	public void hamcrestWithPostExtractor(){
		assertEquals("world",Matching.whenValues(c->c.isMatch(containsInAnyOrder("world","hello"))
