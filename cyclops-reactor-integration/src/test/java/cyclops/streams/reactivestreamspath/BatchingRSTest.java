...
public class BatchingRSTest {
	...
	public void batchUntil(){
		assertThat(Fluxs.just(1,2,3,4,5,6)
	...
	public void batchWhileCollection(){
		System.out.println("*"+ Fluxs.just(1,2,3,4,5,6)
	...
	public void batchByTime2(){
		for(int i=0;i<5;i++){
			...
			assertThat(Fluxs.of(1,2,3,4,5, 6)
							.map(n-> n==6? sleep(1) : n)
	...
	public void xPer(){
		SimpleTimer timer = new SimpleTimer();
		System.out.println(of(1,2,3,4,5,6).xPer(6,1000,TimeUnit.NANOSECONDS).to(Streamable::fromStream).collect(Collectors.toList()));
