...
public interface Ior<ST,PT> extends Supplier<PT>,Value<PT>,BiFunctor<ST,PT>{
	...
	public static <ST,PT> Ior<ST,PT> secondary(ST secondary,PT primary){
		return new Both<>(primary,secondary);
	}
	...
	@AllArgsConstructor(access=AccessLevel.PRIVATE)
	public static class Both<ST,PT>{
		private final Primary<ST,PT> primary;
		private final Secondary<ST,PT> secondary;
	}
}